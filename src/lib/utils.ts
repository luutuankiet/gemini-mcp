...
export function mcpProxy({
        ...
        return {
          ...
          result: {
            ...
            tools: res.result.tools
              .filter((tool: any) => shouldIncludeTool(ignoredTools, tool.name))
              .map((tool: any) => ({
                ...tool,
                inputSchema: tool.inputSchema ? makeGeminiCompatible(tool.inputSchema) : tool.inputSchema,
              })),
          },
        }
      }
  ...
  transportToClient.onmessage = (_message) => {
    ...
    if (message.method === 'initialize') {
      ...
      if (clientInfo) clientInfo.name = `${clientInfo.name} (via gemini-mcp, mcp-remote ${MCP_REMOTE_VERSION})`
